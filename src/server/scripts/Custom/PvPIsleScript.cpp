        ...
        void OnPVPKill(Player* killer, Player* victim)
        {
            if (killer->GetAreaId() == AREA_PVP_ISLAND)
            {
                if (victim->GetAreaId() == AREA_PVP_ISLAND)
                {
					...
					if (killerGUID != victimGUID)
					{
						uint8 honor = (irand(96,150) * sWorld->getRate(RATE_HONOR)) + 24;
        ...
        struct npc_summon_powerupsAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff)
            {
                if (summonTimer <= diff)
                {
					...
					if (i == itwo)
						itwo = urand(0, MAX_POWERUP_SPAWN_POINTS - 1);
                    me->SummonGameObject(GAMEOBJECT_FOOD, powerSpawnPoint[itwo].GetPositionX(), powerSpawnPoint[itwo].GetPositionY(), powerSpawnPoint[itwo].GetPositionZ(), powerSpawnPoint[itwo].GetOrientation(), powerSpawnPoint[itwo].GetOrientation(), powerSpawnPoint[itwo].GetOrientation(), powerSpawnPoint[itwo].GetOrientation(), powerSpawnPoint[itwo].GetOrientation(), 75000);
                   
				   ithree = urand(0, MAX_POWERUP_SPAWN_POINTS - 1);
					if (i == ithree || itwo == ithree)
						ithree = urand(0, MAX_POWERUP_SPAWN_POINTS - 1);
					 me->SummonGameObject(GAMEOBJECT_ANCIENT, powerSpawnPoint[ithree].GetPositionX(), powerSpawnPoint[ithree].GetPositionY(), powerSpawnPoint[ithree].GetPositionZ(), powerSpawnPoint[ithree].GetOrientation(), powerSpawnPoint[ithree].GetOrientation(), powerSpawnPoint[ithree].GetOrientation(), powerSpawnPoint[ithree].GetOrientation(), powerSpawnPoint[ithree].GetOrientation(), 50000);
